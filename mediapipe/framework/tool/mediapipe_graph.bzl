...
load("@mediapipe_repository//mediapipe/framework:encode_binary_proto.bzl", "encode_binary_proto", "generate_proto_descriptor_set")
load("@mediapipe_repository//mediapipe/framework:transitive_protos.bzl", "transitive_protos")
load("@mediapipe_repository//mediapipe/framework/deps:expand_template.bzl", "expand_template")
...
def mediapipe_binary_graph(name, graph = None, output_name = None, deps = [], testonly = False, **kwargs):
    ...
    native.cc_binary(
        ...
        deps = [
            "@mediapipe_repository//mediapipe/framework/tool:text_to_binary_graph",
...
def data_as_c_string(
    ...
    native.genrule(
        ...
        srcs = srcs,
        outs = outs,
        cmd = "$(location @mediapipe_repository//mediapipe/framework/tool:encode_as_c_string) \"$<\" > \"$@\"",
        tools = ["@mediapipe_repository//mediapipe/framework/tool:encode_as_c_string"],
        testonly = testonly,
    )
...
def mediapipe_simple_subgraph(
    ...
    expand_template(
        ...
        template = "@mediapipe_repository//mediapipe/framework/tool:simple_subgraph_template.cc",
        out = name + "_linked.cc",
        substitutions = {
    ...
    native.cc_library(
        ...
        deps = [
            ...
            "@mediapipe_repository//mediapipe/framework:subgraph",
        ] + deps,
        alwayslink = 1,
